...
function DrawImguiFrame()
  ...
  if imgui.Button("ramizer " .. dec2hex(0x801203A4+resume)) then jfmsu(mem, 0x801203A4) end
  ...
  if imgui.BeginTabBar("MyTabBar") then
    ...
    if imgui.BeginTabItem("Values") then
      ...
      drawSliderLoop(mem, ashleySize , 'Size', 'int16_t*', 512, 14000, 2)
    ...
    if imgui.BeginTabItem("Items") then
      ...
      if imgui.BeginListBox( "##Item" ) then
        for i=0,255,4 do
          id = itemIdPtr[i]
          if id < 0x43 then endItemList = i; break end
          imgui.SetNextItemWidth(80);
          drawInput(mem, itemQtdPtr+i , items_t[ id ], 'uint8_t*' ) 
        end
        imgui.EndListBox()
      end
      ...
      if imgui.BeginCombo( "##New item", "Add a new item:" ) then
        for k, v in pairs(items_t) do
          if imgui.Selectable( v ) then itemIdPtr[endItemList] = k; itemQtdPtr[endItemList] = 1 end
        end
        imgui.EndCombo()
      end
